...
> 当前源码已接入 v0.4 混合语义链路，但本地 fixture 通过仍不代表真实站点已经适配。真实知乎 Moka、阿里 Kuma、
> 中国电信用友大易等页面的验收路线见 [需求文档](./REQUIREMENTS.md) 和
...
- **一键填写**：popup 或 **Alt+Shift+F**；规则为全部字段生成 top-N 候选，LLM 用一次请求复审全页面标准字段和长尾字段，本地适配器执行并最终重新扫描读回；自动补齐教育/项目/奖项等重复条目；支持原生控件和 antd/Element/Arco/Moka 风格下拉、级联省市区、拆分日期、ant-picker、Quill 富文本；只填不提交
...
- **混合语义性能与诊断**：每次填写最多 1 次全页面语义复审；模型不输出浏览器工具，漏项或非法单项只对该字段回退规则/人工；Popup 和侧栏分别显示 LLM/规则映射及 mapped → written → committed → verified，只有最终读回一致才算已填。
...
E2E 覆盖：单次混合语义消息链 → 扫描分区 → 规则候选和 LLM 全量复审 → 原生控件+自定义组件 → 自动点「添加」多条目 →
开放题不瞎填 → DOCX/PDF 导入 → LLM 抽取校对存档 → itSkills→自我评价兜底 → 手动编辑回归 →
最终重新扫描读回 → 文档导入与旧路径回归。全部无界面运行（Playwright Chromium + `--headless=new`；正式版 Chrome 137+ 已禁用
`--load-extension`，自动化一律用 Playwright 自带 Chromium）。调试想看界面：`HEADED=1 npm run e2e`。

安装到 Chrome：
1. 打开 `chrome://extensions`，右上角开启「开发者模式」；
2. 「加载已解压的扩展程序」→ 选择本项目的 `dist/` 目录；已有安装在重新构建后点击扩展卡片的“重新加载”，并刷新目标招聘页面。
...
```
src/
├── shared/      # Profile/PageModel/FormPageIR/SemanticPlan、校验、存储、站点识别
├── background/  # SW：消息路由 + 单次全页面 LLM 语义复审
├── content/     # 页面发现、EntryRoute、规则候选、本地组件执行、最终读回、诊断面板

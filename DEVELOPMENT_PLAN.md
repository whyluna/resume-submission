...
- [ ] 按分区构造请求；
- [ ] 每个请求携带 entryId、fieldId、controlKind、选项和规则候选；
- [ ] 启用 API 时复审全部 eligible 字段；
- [ ] 输出严格 JSON Schema；
- [ ] 支持 keep-rule/replace-rule/fill/manual/skip；
- [ ] 支持选择白名单 transform；
...
- [ ] “至今”开关；
- [ ] 项目职责与项目描述区分；
- [ ] Moka Portal 弹层关联和选中状态读取。

### 9.2 验收

- [ ] fixture 全绿；
- [ ] 知乎真实页面只填不保存；
- [ ] 规则阶段、LLM阶段和执行阶段分别可观察；
- [ ] 学校/专业均真正选中，页面不残留弹层；
- [ ] 教育、项目和获奖日期正确；
- [ ] 无多余卡片；
- [ ] 保存按钮未点击。
...
- [ ] 中国电信 fixture 覆盖 100+ 控件且不截断；
- [ ] 至少一个银行公开表单 fixture 通过；
- [ ] 至少一个央企公开表单 fixture 通过；
- [ ] 中国电信登录后真实页只填不保存验证；
- [ ] selectpicker 可见状态与底层值一致；
- [ ] 分区保存、暂存、提交动作零点击。
...
- [ ] README 支持声明只列 live-verified 平台；
- [ ] 完成隐私说明和故障排查文档；
- [ ] 生成发布候选包并完成真实页面回归。
